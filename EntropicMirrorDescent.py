...
class EntropicMirrorDescent:

    def __init__(self, i, lnprob, D, thetas_init, alpha, T, N, J_t, h_t, nb_samples_y_t, eta_n, bool_average):
        '''
        ...
        :param N: number of iterations in the Entropic Mirror Descent (int)
    ...
    def iterationOne(self, theta, weights):
        ...
        bmu_alpha = self._compute_bmu_one(weights, theta)
    ...
    def _weights_update(self, weights, theta):
        ...
        if self.alpha == 1.:
            W, renyi_bound = self.iterationOne(theta, weights)
