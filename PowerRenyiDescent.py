...
class PowerRenyiDescent:
    ...
    def __init__(self, i, lnprob, D, thetas_init, alpha, T, N, J_t, h_t, nb_samples_y_t, eta_n, bool_md, bool_average,
        ...
        :param bool_md: if True, performs Renyi Descent. if False, performs Power Descent (boolean)
        ...
        :param bool_main_on: if True, numerical experiments with Exploration step from [19]
                             if False, numerical experiments with Exploration step from Appendix D.3.2
        :param cte: constant kappa (float such that (alpha-1) cte \geq 0)
        '''
        # Model
        self.lnprob = lnprob
        self.D = D
        ...
        if bool_main_on:
            self._update_component_parameters = self._thetas_update_sto_perturbation
        else:
            self._update_component_parameters = self._compute_gradient_mean
    ...
    def _compute_propto_bmu_alpha(self, weights, theta):
        ...
        for j in range(self.J_t):
            ...
            for _ in range(nb):
                ...
                propto_bmu_alpha += 1 / q_y * np.exp((self.alpha - 1) * (np.log(muk_y) - lnprob)) * k_theta_y

        return propto_bmu_alpha
    ...
    def iterationAlpha(self, theta, weights):
        ...
        propto_bmu_alpha = self._compute_propto_bmu_alpha(weights, theta)
        mu_bmu_alpha = np.sum(weights * propto_bmu_alpha)

        if self.bool_md:
            intermediate = - self.eta_n / (self.alpha - 1) * np.divide(propto_bmu_alpha, mu_bmu_alpha + self.cte)
            intermediate_max = max(intermediate)

            W = weights * np.exp(intermediate - intermediate_max)

        else:
            W = weights * np.power(propto_bmu_alpha + self.cte, self.eta_n / (1 - self.alpha))

        renyi_bound = 1 / (1 - self.alpha) * np.log(1 / self.nb_samples_y_t * mu_bmu_alpha)

        return W, renyi_bound
    ...
    def _weights_update(self, weights, theta):
        ...
        if not self.alpha == 1.:
            W, renyi_bound = self.iterationAlpha(theta, weights)

        # Renormalise the weights
        w = np.sum(W)

        return W / w, renyi_bound

    def AlphaGammaDescent_algorithm(self, thetas, weights_init):

        self.eta_n = self.eta_n0/np.sqrt(self.N)
        weights, renyi_bound = self._weights_update(weights_init, thetas)
        ...
        while n_iter < self.N:
            ...
            self.eta_n = self.eta_n0/np.sqrt(self.N) #(1 + np.sqrt(n_iter)) #np.sqrt(self.N)
            weights, renyi_bound = self._weights_update(weights, thetas)
        ...
        return weights, renyi_bound_lst
    ...
    def _compute_gradient_mean(self, weights, thetas, previous_J_t):
        gradient_update = np.zeros((self.J_t, self.D))
        gradient_update_sum = np.zeros(self.J_t)
        ...
        for j in range(self.J_t):
            nb = repartition[j]
            for _ in range(nb):
                y = self._generate_from_multivariate(thetas[j], self.h_t, 1)[0]

                k_theta_y = multivariate_normal.pdf(thetas, mean=y, cov=self.h_t * np.identity(self.D))
                muk_y = sum([weights[i] * k_theta_y[i] for i in range(self.J_t)])
                lnprob = self.lnprob(y)

                q_y = muk_y

                gradient_update_aux = 1 / q_y * np.exp((self.alpha - 1) * (np.log(muk_y) - lnprob)) * k_theta_y
                gradient_update += [gradient_update_aux[l] * y for l in range(self.J_t)]
                gradient_update_sum += gradient_update_aux

        gradient_mean = gradient_update / gradient_update_sum.reshape(-1, 1)

        return np.reshape(gradient_mean, (-1, self.D))
    ...
    def _full_algorithm(self):
        '''
        Complete algorithm with Exploitation and Exploration steps
        :return: final thetas, final weights, list of all computed renyi_bounds
        '''
        thetas = self.thetas_init.copy()

        # Init with np.array([1 / self.J_t] * self.J_t) and perform AlphaGammaDescent
        weights, renyi_bound = self.AlphaGammaDescent_algorithm(thetas, np.array([1 / self.J_t] * self.J_t))

        renyi_bound_lst = renyi_bound

        previous_J_t = self.J_t
        thetas = self._update_component_parameters(weights, thetas, previous_J_t)
        ...
        return thetas, weights, renyi_bound_lst
